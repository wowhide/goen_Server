...
/**
 * ご縁アプリプレミアム版管理システムを制御するコントローラクラス
 *
 * LICENSE:
 *
...
class MngController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
        if ($this->chkSession() === true) {
            //ログイン済みの場合
            //故人様一覧画面表示処理に遷移
            return $this->_forward('dispmemberlist');
        }
        ...
        if ($this->checkLogin($cookieId, $cookiePassword)) {
            ...
            // $this->_redirect('https://memorial-site-net.ssl-xserver.jp/iei/login.php');  
        }
    }
    ...
    private function checkLogin($id, $password)
    {
        ...
        //引数のIDを条件に、アプリID
        $appliid = $this->_mngModel->getManager(comEncryption::encryption($id));

        //引数のIDを条件に、管理者情報を取得する
        $manager = $this->_mngModel->getManagerId(comEncryption::encryption($id),$appliid['appli_id']);
        ...
        // アプリIDをキーに葬儀社情報にアクセス
        $mortician = $this->_mngModel->getMortician($appliid['appli_id']);
        //削除フラグが-1の場合、「false」を返す
        if ($mortician['del_flg'] == -1) {
            return false;
        }
        ...
        //セッションにアプリIDを設定
        $this->_session->appli_id       = $manager['appli_id'];
        //セッションに権限を設定
        $this->_session->manager_auth   = $manager['manager_auth'];
...
//=====================会員様情報======================//
    //会員様一覧画面表示
    public function dispmemberlistAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        ...
        $this->_session->search_member_join_date_list_from = "";
        $this->_session->search_member_join_date_list_to = "";
        $this->_session->search_member_name = "";
        $this->_session->search_member_mailaddress = "";
        $this->_session->page_member_list = 1;
        $appli_id = $this->_session->appli_id;
        $manager_auth = $this->_session->manager_auth;

        $this->dispMemberList("", "","", "", 1,$appli_id,$manager_auth);
    }

    //会員様一覧画面定期更新
    public function reloadmemberlistAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
        ...
        $searchFrom = $this->_session->search_member_join_date_list_from;
        $searchTo = $this->_session->search_member_join_date_list_to;
        $searchMemberName = $this->_session->search_member_name;
        $searchMemberMailAddress = $this->_session->search_member_mailaddress;
        $pagenum = $this->_session->page_member_list;

        //DBから会員情報を取得
        $memberInfoList = $this->_mngModel->getMemberList($searchFrom, $searchTo,$searchMemberName,$searchMemberMailAddress);

        //ページング処理
        //ページネーターを取得
        $paginator = new Zend_Paginator(new Zend_Paginator_Adapter_Array($memberInfoList));
        //現在ページを設定
        $paginator->setCurrentPageNumber($pagenum);
        //1ページあたりの表示件数を設定
        $paginator->setItemCountPerPage(DECEASED_LIST_NUMBER);

        //テンプレートにページネーター（会員様一覧）を設定
        $this->_view->memberInfoList = $paginator;
        //ページ情報を設定
        $pager = $paginator->getPages();
        $this->_view->total = $pager->totalItemCount;                // 全データ数
        $this->_view->all = $pager->pageCount;                       // 全ページ数
        ...
        echo $this->_view->render('mng_member_list_reload.tpl');
    }


    //会員様一覧検索表示
    public function dispmembersearchAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        ...
        $searchMemberName = $this->getRequest()->getParam('search_member_name', "");
        $searchMemberMailAddress = $this->getRequest()->getParam('search_member_mailaddress', "");
        ...
        if ($this->getRequest()->getParam('clear')) {                //クリアボタンの場合
            ...
            $searchTo = "";
            $searchMemberName = "";
            $searchMemberMailAddress = "";
        }

        //セッションに検索条件とページを設定
        $this->_session->search_member_join_date_list_from = $searchFrom;
        $this->_session->search_member_join_date_list_to = $searchTo;
        $this->_session->search_member_name = $searchMemberName;
        $this->_session->search_member_mailaddress = $searchMemberMailAddress ;
        $this->_session->page_member_list = 1;
        $appli_id = $this->_session->appli_id;
        $manager_auth = $this->_session->manager_auth;

        $this->dispMemberList($searchFrom, $searchTo, $searchMemberName,$searchMemberMailAddress,1,$appli_id,$manager_auth);
    }

    //会員様一覧画面戻る表示
    public function dispmemberreturnAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        ...
        $searchMemberName = $this->_session->search_member_name;
        $searchMemberMailAddress = $this->_session->search_member_mailaddress;
        $page = $this->_session->page_member_list;
        $appli_id = $this->_session->appli_id;
        $manager_auth = $this->_session->manager_auth;
    ...
    //会員様一覧画面ページ指定表示
    public function dispmemberagingAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        ...
        //セッションにページを設定
        $this->_session->page_member_list = $page;
     ...
     */
    private function dispMemberList($searchFrom, $searchTo, $searchMemberName,$searchMemberMailAddress,$pagenum,$appliId,$managerAuth) {
        //検索条件を画面に設定
        $this->_view->searchFrom = $searchFrom;
        $this->_view->searchTo = $searchTo;
        $this->_view->searchMemberName = $searchMemberName;
        $this->_view->searchMemberMailAddress = $searchMemberMailAddress;
        $this->_view->appliId               = $appliId;
        $this->_view->managerAuth           = $managerAuth;

        //DBから会員情報を取得
        $memberInfoList = $this->_mngModel->getMemberList($searchFrom, $searchTo,$searchMemberName,$searchMemberMailAddress);

        //DBから葬儀社情報を取得
        $mortician = $this->_mngModel->getMortician($appliId);

        //葬儀社名をセッションに保存
        $this->_session->morticianName  = $mortician['mortician_name'];
        $this->_view->morticianName     = $mortician['mortician_name']; 
        //権限をビューにセット
        $this->_view->managerAuth = $managerAuth;
        ...
        //セッションから管理者キーを取得＿ビューにセット
        $adminKey = $this->_session->admin_key;
        if (!empty($adminKey)) {
            $this->_view->adminKey = $adminKey;             
        }

        echo $this->_view->render('mng_member_list.tpl');
    }

    //会員様QR発注フォーム画面表示
    public function dispmemberqrorderAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        $memberInfo = array(
            ...
            'allow_push' => 1,
            'cachekey' => "");
        $this->dispMemberQrOrder($memberInfo);
    }

    /**
     * 会員様QR発注フォーム画面表示処理
     * ：会員様QR発注フォームを表示する
     * @param array $memberInfo     入力した会員様情報
     * @param string $message       エラーメッセージ
     */
    private function dispMemberQrOrder(array $memberInfo, $message = "") {
        //ビューに値を設定する
        $this->_view->message               = $message;
        $this->_view->appliId               = $this->_session->appli_id;
        $this->_view->memberLastName        = $memberInfo['member_last_name'];
        $this->_view->memberFirstName       = $memberInfo['member_first_name'];
        $this->_view->memberBirthdayY       = $memberInfo['member_birthday_y'];
        $this->_view->memberBirthdayM       = $memberInfo['member_birthday_m'];
        $this->_view->memberBirthdayD       = $memberInfo['member_birthday_d'];
        $this->_view->memberPostalcodeF     = $memberInfo['member_postalcode_f'];
        $this->_view->memberPostalcodeR     = $memberInfo['member_postalcode_r'];
        $this->_view->memberTelF            = $memberInfo['member_tel_f'];
        $this->_view->memberTelC            = $memberInfo['member_tel_c'];
        $this->_view->memberTelR            = $memberInfo['member_tel_r'];
        $this->_view->memberMailaddress     = $memberInfo['member_mailaddress'];
        $this->_view->memberEntrypassword   = $memberInfo['member_entrypassword'];
        $this->_view->checked5 = "";
        $this->_view->checked6 = "";

        switch ($memberInfo['allow_push']) {
            case 1:
        ...
        //================== タイトル表示 =====================//
        //権限をビューにセット
        $this->_view->managerAuth = $this->_session->manager_auth;
        //セッションから管理者キーを取得＿ビューにセット
        $adminKey = $this->_session->admin_key;
        if (!empty($adminKey)) {
            $this->_view->adminKey = $adminKey;            
        ...
        //葬儀社名をセッションに保存
        $this->_view->morticianName     = $this->_session->morticianName;
        
        echo $this->_view->render('mng_member_qr_order.tpl');
    }


    //会員QR発注フォーム確認画面表示
    public function confmemberqrorderAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        ...
        if ($this->getRequest()->getPost('back')) {
            //故人様一覧画面を表示
            return $this->dispmemberlistAction();
        }

        //入力値を取得
        $memberInfo = 
        array(
            'member_last_name'      => common::delSpace($this->getRequest()->getPost('member_last_name')),
            'member_first_name'     => common::delSpace($this->getRequest()->getPost('member_first_name')),
            'member_birthday_y'     => $this->getRequest()->getPost('member_birthday_y'),
            'member_birthday_m'     => $this->getRequest()->getPost('member_birthday_m'),
            'member_birthday_d'     => $this->getRequest()->getPost('member_birthday_d'),
            'member_postalcode_f'   => $this->getRequest()->getPost('member_postalcode_f'),
            'member_postalcode_r'   => $this->getRequest()->getPost('member_postalcode_r'),
            'member_tel_f'          => $this->getRequest()->getPost('member_tel_f'),
            'member_tel_c'          => $this->getRequest()->getPost('member_tel_c'),
            'member_tel_r'          => $this->getRequest()->getPost('member_tel_r'),
            'member_address'        => "",
            'member_mailaddress'    => $this->getRequest()->getPost('member_mailaddress'),
            'member_entrypassword'  => $this->getRequest()->getPost('member_entrypassword'),
            'allow_push'            => $this->getRequest()->getPost('allow_push')
            );

        //入力チェック
        $message = $this->checkMemberInfo($memberInfo);
        if (comValidate::chkNotEmpty($message) === false) {
            ...
            $this->_view->memberBirthday = $memberBirthday;
            //郵便番号
            $memberPostalcode = $memberInfo['member_postalcode_f'] . "-" .
                    sprintf("%04d", $memberInfo['member_postalcode_r']);
            $this->_view->memberPostalcode = $memberPostalcode;
            //住所(郵便番号から取得)
            $member_address = $memberInfo['member_postalcode_f'].sprintf("%04d", $memberInfo['member_postalcode_r']);
            $get_address = $this->_mngModel->getAddressFromPostalcode($member_address);
            $memberInfo['member_address'] = $get_address;
            $this->_view->memberAddress = $get_address;

            //電話番号
            $memberTel = $memberInfo['member_tel_f'] . "-" .
            sprintf("%3d", $memberInfo['member_tel_c']) . "-" .
            sprintf("%04d", $memberInfo['member_tel_r']);
            $this->_view->memberTel = $memberTel;
            //メールアドレス
            $this->_view->memberMailaddress = $memberInfo['member_mailaddress'];
            //パスワード
            $this->_view->memberEntrypassword = $memberInfo['member_entrypassword'];
            ...
            //入力値をセッションに設定
            $this->_session->member_info = $memberInfo;
            ...
            //入力値が正しい場合、確認画面を表示
            echo $this->_view->render('mng_member_qr_order_conf.tpl');
        } else {
            //入力値が不正な場合、入力画面に戻る
            $this->dispMemberQrOrder($memberInfo, $message);
        }
    }
     ...
     * @return string 正しい場合空文字、不正な場合エラーメッセージ
     */
    private function checkMemberInfo(array $memberInfo) {
        $message = "";          //エラーメッセージ格納用
        //会員様名
        //入力されているか
        if (comValidate::chkNotEmpty($memberInfo['member_last_name']) === false ||
            comValidate::chkNotEmpty($memberInfo['member_first_name']) === false) {
            $message = $message . "・会員様名が入力されていません。<br>";
        }
        //生年月日
        $memberBirthday = $memberInfo['member_birthday_y'] . "/" .
                $memberInfo['member_birthday_m'] . "/" . $memberInfo['member_birthday_d'];
        if(comValidate::chkDate($memberBirthday) === false) {    //日付形式が正しいか
            $message = $message . "・生年月日はYYYY/M/D形式（例：2014/3/10）で入力してください。<br>";
        } elseif($memberInfo['member_birthday_y'] < 1900) {
            $message = $message . "・生年月日は1900年以降で入力してください。<br>";
        }

        //郵便番号
        if (comValidate::chkNotEmpty($memberInfo['member_postalcode_f']) === false ||
            comValidate::chkNotEmpty($memberInfo['member_postalcode_r']) === false) {
            $message = $message . "・郵便番号が入力されていません。<br>";
        }else{
                if (comValidate::chkDigits($memberInfo['member_postalcode_f']) === false ||
                comValidate::chkDigits($memberInfo['member_postalcode_r']) === false) {
                $message = $message . "・数値を入力してください。<br>";
            }else{
                if (comValidate::chkStringLength($memberInfo['member_postalcode_f'],3,3) === false ||
                comValidate::chkStringLength($memberInfo['member_postalcode_r'],4,4) === false) {
                $message = $message . "・郵便番号の桁数が正しくありません。<br>";
                }
            }
        }

        //電話番号
        if (comValidate::chkNotEmpty($memberInfo['member_tel_f']) === false ||
            comValidate::chkNotEmpty($memberInfo['member_tel_c']) === false ||
            comValidate::chkNotEmpty($memberInfo['member_tel_r']) === false) {
            $message = $message . "・電話番号が入力されていません。<br>";
        }else{
            if (comValidate::chkDigits($memberInfo['member_tel_f']) === false ||
                comValidate::chkDigits($memberInfo['member_tel_c']) === false ||
                comValidate::chkDigits($memberInfo['member_tel_r']) === false) {
                $message = $message . "・電話番号は数値を入力してください。<br>";
            }else{
                if (comValidate::chkStringLength($memberInfo['member_tel_f'],3,3) === false ||
                    comValidate::chkStringLength($memberInfo['member_tel_c'],3,3) === false ||
                    comValidate::chkStringLength($memberInfo['member_tel_r'],4,4) === false) {
                    $message = $message . "・電話番号の桁数が正しくありません。<br>";
                }
            }
        }
        ...
        //メールアドレス
        if (comValidate::chkNotEmpty($memberInfo['member_mailaddress']) === false) {
            $message = $message . "・メールアドレスが入力されていません。<br>";
        }else{
            if (comValidate::chkEmailAddress($memberInfo['member_mailaddress']) === false) {
            $message = $message . "・メールアドレスの形式が正しくありません。<br>";
            }
        }

        //パスワード
        if (comValidate::chkNotEmpty($memberInfo['member_entrypassword']) === false) {
            $message = $message . "・パスワードが入力されていません。<br>";
        }else{
            if (comValidate::chkAlnum($memberInfo['member_entrypassword']) === false) {
                $message = $message . "・パスワードは半角英数字で入力してください。<br>";
            }else{
                if (comValidate::chkStringLength($memberInfo['member_entrypassword'],4,10) === false) {
                $message = $message . "・パスワードの桁数が正しくありません。<br>";
                }                
            }
        }
    ...
    //QR発注フォーム完了画面表示
    public function compmemberqrorderAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        ...
        //セッションから入力値を取得
        $memberInfo = $this->_session->member_info;
        ...
        if ($this->getRequest()->getPost('back')) {              //戻るボタンの場合
            ...
            do {
                //会員番号を発行
                $memberId = comEncryption::getRandomString(10);
                //重複する会員番号が存在しないか
                $member = $this->_mngModel->getMember($memberId);
                if (!$member) {
                    $duplicateFlg = false;
                }
            } while($duplicateFlg);
            ...
            //DBに会員情報を発行状態コードを発行依頼済みで保存
            if ($this->_mngModel->insertMember($memberInfo)) {
                //画像を選択している場合、一時フォルダから正式なフォルダに移動する
                if ($deceasedInfo['image_existence_flg'] == IMAGE_EXISTENCE_FLG_YES) {
                    //ディレクトリ生成
                    $fileDir = DECEASED_DATA_PATH . date("Y") . '/' . $memberInfo['member_id'];
                    if (mkdir($fileDir, 0755, true)) {
                        //一時フォルダの仮アップ画像を画像フォルダに移動
                        $imagePath = $fileDir . '/' . $memberInfo['member_id'] . '.jpg';
                ...
                $memberInfo['member_name'] =
                        $memberInfo['member_last_name'] . "　" . $memberInfo['member_first_name'];
                //ご縁アプリのご案内を発行する
                comGuidanceIssue::issue($memberInfo, $fileDir);
                ...
                $this->_view->memberName =
                        $memberInfo['member_last_name'] . '　' .
                        $memberInfo['member_first_name'];               //会員様
            ...
            //完了画面を表示
            echo $this->_view->render('mng_member_qr_order_comp.tpl');
        }
    }

    //会員様表示
    public function dispmemberinfoAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        ...
        //GET値から故人IDを取得
        $memberId = $this->getRequest()->getQuery('did');
        ...
        //会員様表示画面表示
        if($this->dispMemberInfo($memberId)){
            echo $this->_view->render('mng_member_info_display.tpl');
        }else{
            echo $this->_view->render('mng_error.tpl');
        }
    }

    private function dispMemberInfo($memberId) {
        //会員情報を取得
        $memberInfo = $this->_mngModel->getMember($memberId);

        if (empty($memberInfo) === false) {
            if ($memberInfo['issue_state_code'] == ISSUE_STATE_CODE_SUBMIT ||
                $memberInfo['issue_state_code'] == ISSUE_STATE_CODE_COMP) {
                //viewを設定
                $this->_view->issueStateCode    = $memberInfo['issue_state_code'];
                $this->_view->issueDatetime     = $memberInfo['issue_datetime'];
                $this->_view->entryDatetime     = $memberInfo['entry_datetime'];
                $this->_view->memberId          = $memberInfo['member_id'];
                $this->_view->memberName        = $memberInfo['member_name'];
                $this->_view->memberBirthday    = $memberInfo['member_birthday'];
                $this->_view->memberTel         = $memberInfo['member_tel'];
                $this->_view->memberPostalcode  = $memberInfo['member_postalcode'];
                $this->_view->memberMailaddress = $memberInfo['member_mailaddress'];
                // switch ($memberInfo['kyonen_gyonen_flg']) {
                //         case 0:
                ...
                //             break;
                // }
                // $this->_view->deathAge          = $memberInfo['death_age'];
                switch ($memberInfo['allow_push']) {
                    case 1:
                        $this->_view->allowPush = "通知する";
                        break;
    ...
    //会員様削除
    public function delmemberinfoAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        ...
        //会員情報を論理削除する
        if ($this->_mngModel->logicDeleteMember($memberId)) {
            //会員情報フォルダのパスを取得
            $member = $this->_mngModel->getMember($memberId);
            $memberDataPath = DECEASED_DATA_PATH .
                    substr($member["issue_datetime"], 0, 4) . '/' . $memberId;
            if (file_exists($memberDataPath)) {
                //ファイルが存在する場合、削除する
                common::removeDirectory($memberDataPath);
            }
            //ログ出力
            $this->_logModel->recordLog(LOG_KIND_DECEASED_LOGIC_DELETE, $this->_session->manager_id, "Success", $this->_httpHeaderInfo);
            //会員様一覧に戻る
            return $this->_forward('dispmemberreturn');
        } else {
            //ログ出力
    ...
    //会員情報編集画面表示
    public function dispeditmemberinfoAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        ...
        $arrayMemberName = common::splitDeceasedName($memberInfo['member_name']);
        $memberInfo['member_last_name'] = $arrayMemberName[0];
        $memberInfo['member_first_name'] = $arrayMemberName[1];
        //編集用に誕生日を年月日に分けて配列に設定
        $memberInfo['member_birthday_y'] = substr($memberInfo['member_birthday'], 0, 4);
        $memberInfo['member_birthday_m'] = substr($memberInfo['member_birthday'], 4, 2);
        $memberInfo['member_birthday_d'] = substr($memberInfo['member_birthday'], 6, 2);
        //編集用に郵便番号を分けて配列に設定
        $memberInfo['member_postalcode_f'] = substr($memberInfo['member_postalcode'], 0, 3);
        $memberInfo['member_postalcode_r'] = substr($memberInfo['member_postalcode'], 4, 4);
        //編集用に電話番号を分けて配列に設定
        $member_tel_pieces = explode("-", $memberInfo['member_tel']);
        $memberInfo['member_tel_f'] = $member_tel_pieces[0];
        $memberInfo['member_tel_c'] = $member_tel_pieces[1];
        $memberInfo['member_tel_r'] = $member_tel_pieces[2];
        //パスワードを復号化
        // $memberInfo['member_entrypassword'] = comEncryption::decryption($memberInfo['member_entrypassword']);

        //会員様編集画面を表示
        $this->dispEditMemberInfo($memberInfo);
    }

    private function dispEditMemberInfo(array $memberInfo, $message = "") {
        //viewを設定
        $this->_view->message               = $message;
        $this->_view->memberLastName        = $memberInfo['member_last_name'];
        $this->_view->memberFirstName       = $memberInfo['member_first_name'];
        ...
        $this->_view->memberMailaddress     = $memberInfo['member_mailaddress'];
        $this->_view->memberEntrypassword   = $memberInfo['member_entrypassword'];
        // $this->_view->memberEntrypassword   = comEncryption::decryption($memberInfo['member_entrypassword']);
        ...
        //画面を表示
        echo $this->_view->render('mng_member_info_edit.tpl');
    }



    //会員様情報編集確認
    public function confeditmemberinfoAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        ...
        if ($this->getRequest()->getPost('back')) {              //戻るボタンの場合
            ...
            //入力チェック
            $message = $this->checkMemberInfo($memberInfo);
            // $message = $message . $this->checkMemberImage();
            if (comValidate::chkNotEmpty($message) === false) {
                //入力値をセッションに設定
                $this->_session->member_info = $memberInfo;
                ...
                $this->_view->token = comToken::get_token($this->_session->key);
                //入力値が正しい場合、確認画面を表示
                echo $this->_view->render('mng_member_info_edit_conf.tpl');
            } else {
                //入力値が不正な場合、入力画面に戻る
                $this->dispEditMemberInfo($memberInfo, $message);
            }
        }
    }
    
    public function compeditmemberinfoAction() {
        if ($this->chkSession() === false) {
            //ログインしていない場合またはセッションタイムアウトした場合、ログイン画面を表示
            return $this->_forward('disprelogin');
        }
        ...
        if ($this->getRequest()->getPost('back')) {              //戻るボタンの場合
            ...
            //更新処理
            if ($this->_mngModel->updateMember($memberInfo)) {
                //遺影写真保存先デイレクトリのフォルダ取得
                $member = $this->_mngModel->getMember($memberInfo['member_id']);

                //遺影写真のパス
                // $imagePath = $fileDir . '/' . $memberInfo['member_id'] . '.jpg';
                // if ($memberInfo['image_existence_flg'] == IMAGE_EXISTENCE_FLG_YES) {
                //     //画像を選択している場合、一時フォルダから正式なフォルダに移動する
                //     //ディレクトリが存在するかチェック
                //     if (file_exists($fileDir)) {
                //         //一時フォルダの仮アップ画像を画像フォルダに移動
                //         if (rename($this->_session->member_temp_data_path, $imagePath) === false) {
                //             //失敗した場合
                //             echo "ファイルを移動出来ませんでした。";
                //         }
                //     } else {
            ...
            //完了画面を表示
            echo $this->_view->render('mng_member_info_edit_comp.tpl');
        }
    }
