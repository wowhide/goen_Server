...
/**
 * かずやアプリプレミアム版との連携機能を制御するコントローラクラス
 *
 * LICENSE:
 *
...
require_once 'application/common/comNoticeMail.php';
...
*/
    ...
    public function getthumbnailAction() {
        ...
        // 画像ファイルのパスを指定
        $src = '../../folder_app_ms-kazuya/photo/' . $deceasedId . '.jpg' ;
        // 画像ファイルを出力
        header('Content-type: image/jpeg');
        readfile($src);
    }
    ...
    public function storeimageAction() {
        ...
        // 画像ファイルのパスを指定
        $src = '../../folder_app_ms-kazuya/store/' . $deceasedId . '.jpg' ;
        // 画像ファイルを出力
        header('Content-type: image/jpeg');
        readfile($src);
    }
...
//        $deceasedId = $this->_request->getParam('deceased_id');
    ...
    public function getnoticebynumberAction() {
        ...
        if(strcmp($target["entry_method"], '1') == 0) {
            ...
            $result .= $_SERVER["HTTP_HOST"].'/ms-kazuya/mng/viewnoticeinfo?nino='.$notice_no;
